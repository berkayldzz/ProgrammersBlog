...
namespace ProgrammersBlog.Services.Concrete
{
    ...
    {
        // Category entitymiz ile ilgili işlemleri yöneteceğimiz sınıf.
        // Burdaki işlemleri yapabilmemiz için veritabanı ile iletişimde olmamız gerek.Bunun için UnitOfWork yapısını kurmuştuk.

        // ManagerBase sınıfını kullanıyoruz artık.
        // private readonly IUnitOfWork _unitOfWork; // Constructor içine enjekte etmeliyiz.
        ...
        public async Task<IDataResult<CategoryDto>> AddAsync(CategoryAddDto categoryAddDto, string createdByName)
        {
            ...
            await UnitOfWork.SaveAsync();    // Bu işlem ile kategori veritabanına kaydediliyor.

            // ContinueWith:yeni bir task ile zincirleme olarak devam et.İşlemlerin daha hızlı olmasını sağlıyor.Yukarıdaki eklemeyi bitirdiği gibi bu işleme geçiyor
        ...
        {
            var category = await UnitOfWork.Categories.GetAsync(c => c.Id == categoryId); // Sileceğimiz değeri getirmemiz gerek.
            if (category != null)
            {
        ...
        public async Task<IDataResult<CategoryDto>> GetAsync(int categoryId)
        {
            ...
            // Bir kategoriyi buraya getirmemiz gerekiyor.
            // _unitOfWork. dediğimiz zaman tüm repositorylere ulaşabiliyoruz.
            // En son bizden bir predicate istiyor yani filtre.Sen bana bir filtre ver ki ben ona göre senin istediğin entityi getiriyim.

            var category = await UnitOfWork.Categories.GetAsync(c => c.Id == categoryId); // Bu id ye eşit entityi getir.
        ...
        public async Task<IDataResult<CategoryListDto>> GetAllAsync()
        {
            ...
            if (categories.Count > -1) // Listemizde 0 tane kategori de olabilir.
            {
        ...
        {
            var result = await UnitOfWork.Categories.AnyAsync(c => c.Id == categoryId); // Böyle bir veri var ise
            if (result)
            {
            ...
            else // Böyle bir categoryid ye sahip category yoksa
            {
        ...
        public async Task<IResult> HardDeleteAsync(int categoryId)
        {
            ...
            if (category != null)
            {
                await UnitOfWork.Categories.DeleteAsync(category);  // Kategoriyi tamamen siliyor olacağız.Veritabanından siliyor olacağız.
                await UnitOfWork.SaveAsync();
